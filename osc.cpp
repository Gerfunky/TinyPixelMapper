	...
	if  ((osc_out_float_value.isEmpty() != true || osc_out_rgb_value .isEmpty() != true || osc_out_SelVal_value.isEmpty() != true || osc_out_int_value.isEmpty() != true) )
	{
		...
		if (osc_out_float_value.count() != 0 ||  (osc_out_rgb_value.count()) != 0 ||  (osc_out_SelVal_value.count())   != 0 ||  (osc_out_int_value.count())   != 0  || osc_out_Stringvalue.count() !=0)
		{
			...
			&& !get_bool(FFT_OSTC_VIZ)) 
					{bundle_out.add("/app/refMix").add(0);
						bundle_out.add("/ostc/master/connled").add(0).add(255).add(0).add(255); }// each bundle should refresh the led. when the buffers are amty and were not refreshing all send green
...
void osc_StC_menu_form_modify_adv_ref(uint8_t bit)
{
		...
		for (uint8_t bit_formNr = 0; bit_formNr < 8; bit_formNr++)
		{
			...
			osc_queu_MSG_int( "/ostc/form/mir/run/" + String(formNr),	(bitRead(deck[0].cfg.form_menu_modify[bit][_M_FORM_MODIFY_MIRROR], 		bit_formNr)));  	
			osc_queu_MSG_int( "/ostc/form/mir/rev/" + String(formNr),	(bitRead(deck[0].cfg.form_menu_modify[bit][_M_FORM_MODIFY_MIRROR_REVERSED], 		bit_formNr)));  
...
void osc_StC_menu_form_pal_adv_ref(uint8_t bit)
{
		...
		for (uint8_t bit_formNr = 0; bit_formNr < 8 ; bit_formNr++)
		{
			...
			osc_queu_MSG_int("/ostc/form/pal/aat/" + String(bit), Ap_MANUAL );
...
void osc_StC_menu_pal_ref(uint8_t pal) 
{
	...
	for (int i = 0; i < 16; i++) 	osc_queu_MSG_rgb( String("/ostc/pal/1/" + String(i) ) ,  LEDS_pal_read(pal,i,0), LEDS_pal_read(pal,i,1), LEDS_pal_read(pal,i,2) );	
...
osc_queu_MSG_int("/ostc/master/playNr", led_cfg.Play_Nr);
	for(uint8_t confNr = 0; confNr < MAX_NR_SAVES ; confNr++)  		// update leds to show what confs are saved
	{
		...
		if(FS_check_Conf_Available(confNr) == false) osc_queu_MSG_rgb(String("/ostc/master/conf/l/"+String(confNr)  ), uint8_t(255),uint8_t(0),uint8_t(0));
		else										 osc_queu_MSG_rgb(String("/ostc/master/conf/l/"+String(confNr)  ), uint8_t(0),uint8_t(255),uint8_t(0));
...
void osc_StC_form_routing(OSCMessage &msg, int addrOffset)
{
		...
		else if 	(msg.match("/fx",addrOffset))	
		{
			...
			else 
			{
				...
				else if		(msg.match("/fx/fx01/lvb",addrOffset))		{	deck[0].fx1_cfg.form_fx1[orig_form_nr].lvl_bin  =  uint8_t(msg.getInt(0))  ;}
		...
		else 
		{
			...
			if(address[4] == '/')	
			{
						...
						if (result < 0 ) result = 4096 + result;  
						debugMe(result);
						//Form Pallete input
						...
						else if		(msg.match("/fft/lvl",addrOffset))		deck[0].cfg.form_fx_fft[orig_form_nr].level  =  uint8_t(msg.getInt(0))  ;
						...
						else if		(msg.match("/mir/run",addrOffset))			{ bitWrite(deck[0].cfg.form_menu_modify[i_bit_int][_M_FORM_MODIFY_MIRROR], 			i_form_nr, 	bool(msg.getInt(0)));  }
						else if		(msg.match("/mir/rev",addrOffset))			{ bitWrite(deck[0].cfg.form_menu_modify[i_bit_int][_M_FORM_MODIFY_MIRROR_REVERSED], 			i_form_nr, 	bool(msg.getInt(0)));  }
...
void osc_StC_master_routing(OSCMessage &msg, int addrOffset) 
{
		//debugMe("in master routing");
			...
			else if (msg.fullMatch("/potsense",addrOffset))			{ led_cfg.PotSens = constrain(uint8_t(msg.getInt(0) ),0,16)  ;}
			else if (msg.fullMatch("/LoadNames",addrOffset) 		&& boolean(msg.getInt(0)) == true)			{ FS_play_conf_readSendSavenames( ) ;}
			else if (msg.fullMatch("/LoadCustNames",addrOffset) 	&& boolean(msg.getInt(0)) == true)			{ FS_play_conf_custom_readSendSavenames( ) ;}
			...
			else if (msg.fullMatch("/ManRef",addrOffset))    	{ write_bool(MANUAL_REFRESH,	bool(msg.getInt(0) )) ;   }
			...
			{ 
				...
				if (length >= 32 )	msg.getString( 0,	led_cfg.Led_Setup_confname,  		sizeof(led_cfg.Led_Setup_confname), 0 , sizeof(led_cfg.Led_Setup_confname)-1 ) ;
					...
					|| (msg.match("/clod",addrOffset))
					|| (msg.match("/cloP",addrOffset))
					|| (msg.match("/csav",addrOffset))
			) 
			{
							char address[5] ;
							...
							else if (boolean(msg.getInt(0))== true)  // if pushdown only
							{
								

								if 			(msg.match("/save",addrOffset))		 { LEDS_G_LoadSAveFade(true ,sel_save_no) ;  } // osc_queu_MSG_VAL_STRING("/ostc/master/savename/" + String(sel_save_no) , deck[0].cfg.confname ) ;   osc_queu_MSG_rgb(String("/ostc/master/conf/l/"+String(sel_save_no)), 0,255,0);       }
								else if 	(msg.match("/csav",addrOffset))		 { uint8_t CustSAveNr = SaveConf.confCustomLoadNrs[sel_save_no] ;   LEDS_G_LoadSAveFade(true ,CustSAveNr) ; } 

								else if 	(msg.match("/load",addrOffset))		 {  write_bool(FFT_OSTC_VIZ, false);  if (FS_check_Conf_Available(sel_save_no)) LEDS_G_LoadSAveFade(false,sel_save_no) ; }
								else if 	(msg.match("/loaP",addrOffset))		 {  write_bool(FFT_OSTC_VIZ, false);  write_bool(MANUAL_REFRESH, true);  if (FS_check_Conf_Available(sel_save_no)) LEDS_G_LoadSAveFade(false,sel_save_no) ; }
								else if 	(msg.match("/clod",addrOffset))		 {  write_bool(FFT_OSTC_VIZ, false);  uint8_t CustSAveNr = SaveConf.confCustomLoadNrs[sel_save_no] ;  LEDS_G_LoadSAveFade(false,CustSAveNr) ; }
								else if 	(msg.match("/cloP",addrOffset))		 {  write_bool(FFT_OSTC_VIZ, false);   write_bool(MANUAL_REFRESH, true); uint8_t CustSAveNr = SaveConf.confCustomLoadNrs[sel_save_no] ;  LEDS_G_LoadSAveFade(false,CustSAveNr) ; }
...
void osc_app_savename_ref( )
{
		...
		msg_out.add(1);  // reset REfreshButton
...
void osc_app_audio_ref(bool onlyStats  )
{
		...
		if (!onlyStats)
		{
			...
			bundle_out.add("/app/audio/g")
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[1], 0) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[1], 1) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[1], 2) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[1], 3) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[1], 4) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[1], 5) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[1], 6) ));
			bundle_out.add("/app/audio/b")
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[2], 0) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[2], 1) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[2], 2) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[2], 3) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[2], 4) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[2], 5) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu[2], 6) ));
			bundle_out.add("/app/audio/aFps")
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_fps, 0) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_fps, 1) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_fps, 2) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_fps, 3) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_fps, 4) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_fps, 5) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_fps, 6) ));
			bundle_out.add("/app/audio/aBri")
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_bri, 0) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_bri, 1) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_bri, 2) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_bri, 3) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_bri, 4) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_bri, 5) ))
				.add(int(bitRead(deck[0].cfg.fft_config.fft_menu_bri, 6) ));
			
			bundle_out.add("/app/vizIt")  .add(int(get_bool(APP_VIZIT)) ) 
										.add(1); //reset the REfLed
		}
		bundle_out.add("/app/audio/trig")
			.add(deck[0].cfg.fft_config.trigger[0]   )
			.add(deck[0].cfg.fft_config.trigger[1]   )
			.add(deck[0].cfg.fft_config.trigger[2]   )
			.add(deck[0].cfg.fft_config.trigger[3]   )
			.add(deck[0].cfg.fft_config.trigger[4]   )
			.add(deck[0].cfg.fft_config.trigger[5]   )
			.add(deck[0].cfg.fft_config.trigger[6]   );
		bundle_out.add("/app/audio/max")
			.add(deck[0].run.fft_data[0].max  )
			.add(deck[0].run.fft_data[1].max  )
			.add(deck[0].run.fft_data[2].max  )
			.add(deck[0].run.fft_data[3].max  )
			.add(deck[0].run.fft_data[4].max  )
			.add(deck[0].run.fft_data[5].max  )
			.add(deck[0].run.fft_data[6].max  );
		bundle_out.add("/app/audio/avg")
			.add(deck[0].run.fft_data[0].avarage  )
			.add(deck[0].run.fft_data[1].avarage  )
			.add(deck[0].run.fft_data[2].avarage  )
			.add(deck[0].run.fft_data[3].avarage  )
			.add(deck[0].run.fft_data[4].avarage  )
			.add(deck[0].run.fft_data[5].avarage  )
			.add(deck[0].run.fft_data[6].avarage  );
		bundle_out.add("/app/audio/last")
			.add(fft_bin_results[0]  )
			.add(fft_bin_results[1]  )
			.add(fft_bin_results[2]  )
			.add(fft_bin_results[3]  )
			.add(fft_bin_results[4]  )
			.add(fft_bin_results[5]  )
			.add(fft_bin_results[6]  );

		bundle_out.add("/app/audio/fftstats")
			.add(uint8_t(LEDS_get_FPS()))
			//.add(LEDS_get_real_bri())
			.add(deck[0].run.fft.fft_color_result_bri)
			.add(deck[0].run.fft.fft_color_fps )
			.add(LEDS_FFT_get_color_result(0,0))
			.add(LEDS_FFT_get_color_result(0,1))
			.add(LEDS_FFT_get_color_result(0,2))
			
			//.add(deck[0].cfg.fft_config.viz_fps)			
			;
...
void osc_app_menu_ledCfg_ref()
{
	...
	IPAddress ip_out(osc_server.remoteIP());
		...
		bundle_out.add( 1  ); // set refbutton
...
void osc_app_menu_wifi_ref()
{
		...
		bundle_out.add(1); // button ref Wifi to 1
...
void osc_app_menu_master_ref()
{
	...
	uint8_t bri = map(deck[0].cfg.led_master_cfg.bri, 0 , led_cfg.max_bri , 0 ,255);
		bundle_out.add("/app/mainVals")
			.add(BOARD_TYPE)
			.add(__DATE__)
			.add(FW_VERSION)
			
			.add(deck[0].cfg.led_master_cfg.r)
			.add(deck[0].cfg.led_master_cfg.g)
			.add(deck[0].cfg.led_master_cfg.b)
			//.add(deck[0].cfg.led_master_cfg.pal_bri)
			.add(bri)
			.add(deck[0].cfg.led_master_cfg.pal_fps)
			.add(uint8_t(LEDS_get_FPS()))
			//.add(LEDS_get_real_bri())
			.add(int(get_bool(SEQUENCER_ON)))
		...
		bundle_out.add("/app/MainLayers")
			.add(deck[0].cfg.layer.save_lvl[0])
			.add(deck[0].cfg.layer.save_lvl[1])
			.add(deck[0].cfg.layer.save_lvl[2])
			.add(deck[0].cfg.layer.save_lvl[3]);

		bundle_out.add("/app/refMix")
			.add(1);
...
void osc_app_routing(OSCMessage &msg, int addrOffset) 
{
		...
		{  
			...
			FS_play_conf_readSendSavenamesAPPmem( ); }   //           FS_play_conf_readSendSavenames( ); } osc_StC_menu_master_loadsave_ref();	
...
void osc_StC_menu_routing(OSCMessage &msg, int addrOffset) 
{
	...
	switch(select)
	{
		case 0:  osc_StC_menu_master_ref(); break; 
...
void osc_StC_routing(OSCMessage &msg, int addrOffset) 
{
	...
	//osc_queu_MSG_rgb( String("/ostc/master/connled" ) ,  	getrand8() ,getrand8() ,getrand8( )  );	   osc_app_menu_master_ref
...
void osc_api_fx_pal_ref(uint8_t InDeck = 0)
{
		...
		for (uint8_t part = 0; part < NR_FORM_PARTS ; part++)  		outArray[part] = deck[InDeck].cfg.form_fx_pal[part].autoPalMode; 		osc_queu_MSG_fx_Int("/ref/fx/pal/ato", InDeck , outArray);
...
void osc_StC_menu_master_ref()
{
	Refreshloop = 40;
		osc_queu_MSG_int("/ostc/master/fps", 		deck[0].cfg.led_master_cfg.pal_fps);
		osc_queu_MSG_int("/ostc/audio/rfps", 		LEDS_get_FPS());
		osc_queu_MSG_VAL_STRING("/ostc/master/wifi/name", wifi_cfg.APname);
		osc_queu_MSG_int("/ostc/master/boardType", 		BOARD_TYPE);
	//	#define BOARD_TYPE 0
...
void osc_api_fx(OSCMessage &msg, int addrOffset)
 {
	...
	else if	(msg.fullMatch("/pal/ald",addrOffset))	 {if (result < 0 ) result = 4096 - result;  deck[DeckNr].cfg.form_fx_pal[OrigFormNr].index_add_led  		=  uint16_t(result)  ;}
	else if	(msg.fullMatch("/pal/afm",addrOffset))	 {if (result < 0 ) result = 4096 - result;  deck[DeckNr].cfg.form_fx_pal[OrigFormNr].index_add_frame  	=  uint16_t(result)  ; debugMe(result); debugMe(uint16_t(result)); }
	else if	(msg.fullMatch("/pal/sid",addrOffset))	 {if (result < 0 ) result = 4096 - result;  deck[DeckNr].cfg.form_fx_pal[OrigFormNr].index_start  		=  uint16_t(result)  ;}
...
void osc_api_routing(OSCMessage &msg, int addrOffset) 
{
	...
	if (msg.fullMatch("/refreshAll",addrOffset))   osc_api_fx_pal_ref(); 
...
void osc_tosc_refresh()
{
	...
	osc_queu_MSG_VAL_STRING("/tosc/SSID", String(wifi_cfg.ssid));
	osc_queu_MSG_VAL_STRING("/tosc/WPW", String(wifi_cfg.pwd));
	osc_queu_MSG_VAL_STRING("/tosc/WAPNL", String(wifi_cfg.APname));
